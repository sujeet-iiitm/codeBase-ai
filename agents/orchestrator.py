...
from agents.specialist_agents import (
    qa_agent, bug_agent, diagram_agent,
    docs_agent, impact_agent, onboarding_agent
)
...
def qa_node(state: AgentState) -> AgentState:
    try:
        return {**state, "result": qa_agent(state["query"], state["chunks"])}
    except Exception as e:
        return {**state, "error": str(e)}

def bug_node(state: AgentState) -> AgentState:
    try:
        return {**state, "result": bug_agent(state["query"], state["chunks"])}
    except Exception as e:
        return {**state, "error": str(e)}

def diagram_node(state: AgentState) -> AgentState:
    try:
        result = diagram_agent(state["query"], state["chunks"], state.get("diagram_type", "architecture"))
...
def docs_node(state: AgentState) -> AgentState:
    try:
        return {**state, "result": docs_agent(state["query"], state["chunks"])}
    except Exception as e:
        return {**state, "error": str(e)}

def impact_node(state: AgentState) -> AgentState:
    try:
        return {**state, "result": impact_agent(state["query"], state["chunks"])}
    except Exception as e:
        return {**state, "error": str(e)}

def onboarding_node(state: AgentState) -> AgentState:
    try:
        return {**state, "result": onboarding_agent(state["chunks"], state.get("file_tree", []))}
    except Exception as e:
        return {**state, "error": str(e)}
...
def build_graph() -> StateGraph:
    ...
    graph.add_conditional_edges("router", route_intent,
        {"qa":"qa","bug":"bug","diagram":"diagram","docs":"docs","impact":"impact","onboarding":"onboarding"})
...
def run_agent(query: str, chunks: List[Dict], file_tree: List[str] = None) -> Dict:
    app = build_graph()
    state = AgentState(
        query=query, intent=None, diagram_type=None,
        chunks=chunks, file_tree=file_tree or [],
        result=None, diagram_result=None, error=None
    )
